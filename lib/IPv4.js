...
class IPv4 {
  ...
  async fromDomainName(domainName) {
    ...
    const response = await dnsPromises.lookup(domainName, { family: 4 }).catch(err => {
      if (err.code === 'ENOTFOUND') throw new Error('Domain name was not found!');
      throw err;
    });
  ...
  getRandomAddress() {
    ...
    for (let i = 0; i < octetsCount; i++) {
      const randomOctet = Math.floor(Math.random() * (maxDecOctet - minDecOctet + 1)) + minDecOctet;
