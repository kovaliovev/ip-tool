...
class IPv4 {
  ...
  splitNetworkInHalf(network) {
    ...
    const binaryNetwork = this.toBinary(ip);
    const hostAddressStart = prefix + Math.floor(prefix / 8);
    const firstNetwork = ip;
    const secondNetwork = this.fromBinary(
      binaryNetwork.slice(0, hostAddressStart) +
        '1' +
        binaryNetwork.slice(hostAddressStart + 1)
    );
