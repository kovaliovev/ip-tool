...
class IPv4 {
  ...
  networkSubnet(network, subnetsRequiredHostsCount) {
    ...
    let decimalSubnetAddress = this.toDecimal(address);
    ...
    for (const hostsCount of subnetsRequiredHostsCount) {
      ...
      if (!(0 < hostsCount && hostsCount < 4294967295)) {
        throw new RangeError('Invalide hosts count entered!');
      }
      const power = Math.ceil(Math.log2(hostsCount + 2));
      const subnetHosts = 2 ** power;
      const subnetPrefix = 32 - power;
      const subnetAddress = this.fromDecimal(decimalSubnetAddress);
      const subnetNetwork = `${subnetAddress}/${subnetPrefix}`;

      result[subnetNetwork] = subnetHosts;
      decimalSubnetAddress += subnetHosts;
